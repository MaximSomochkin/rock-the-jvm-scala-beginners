...
//      expectMsg(0)
  ...
  "A kinder supervisor" should {
    "not kill children in case it's restarted or escalates failures" in{
      val supervisor = system.actorOf(Props[NoDeathRestartSupervisor], "NoDeathRestartSupervisor")
      supervisor ! Props[FussyWordCounter]
      val child = expectMsgType[ActorRef]
  ...
  "All-for-one supervisor" should{
    "apply all-for-one strategy" in {
      val supervisor = system.actorOf(Props[AllForOneSupervisor], "supervisorAllForOne")
      supervisor ! Props[FussyWordCounter]
      val child = expectMsgType[ActorRef]
...
object SupervisionSpec{
  ...
  class  NoDeathRestartSupervisor extends Supervisor{
