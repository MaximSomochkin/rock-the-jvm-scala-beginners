...
object ChildActorExerscise extends App {
  ...
  class WordCounterMaster extends Actor {
    ...
    def withChildren(childrenRefs: Seq[ActorRef],
      ...
      case text: String =>
        ...
        childRef ! task
        val nextChildIndex = (currentChildIndex + 1) % childrenRefs.length // if reached max length childrenRefs go to 0
        val nextTaskId = currentTaskId + 1
        val newReqestMap = requestMap + (currentTaskId -> originalSender)
        context.become(withChildren(childrenRefs, nextChildIndex, nextTaskId, newReqestMap))
  ...
  val system = ActorSystem("roundRobinWordExercise")
  ...
  /*
    ...
    send Initialize(10) wordCounterMaster
    send "Akka is awesome" to wordCounterMaster
    wcm will send a WordCountTask("...") to one of this children
