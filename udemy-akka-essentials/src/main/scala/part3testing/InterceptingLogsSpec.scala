...
class InterceptingLogsSpec extends TestKit(ActorSystem("InterceptingLogScpec", ConfigFactory.load().getConfig("interceptingLogMessages")))
  with ImplicitSender
  ...
  "a checkout flow" should {
    "correctly log dispach of an order" in{
...
object InterceptingLogsSpec {
  ...
  class CheckoutActor extends Actor {
    ...
    def pendingPayment(item: String): Receive = {
      ...
      case PatmentDenied => throw new RuntimeException("I can't handle this anymore")
    }
  ...
  class PaymentManager extends Actor {
    override def receive: Receive = {
      case AuthorizeCard(card) =>
        if (card.startsWith("0")) sender() ! PatmentDenied
        else {
          Thread.sleep(4000)
